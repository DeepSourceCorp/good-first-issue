...
// FIX: Correct issue count logic
const issuesDisplay = computed(() => {
  /*
     Old logic:
    props.repo.issues.length
    → Only counts fetched issues (like 10)

    New logic:
    Use total issue count if available (issues_count)
    Else fallback to fetched issues length
  */
